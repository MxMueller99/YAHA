        ...
        if (json == NULL) {
            ...
            if (main != NULL) {
                ...
                if (cJSON_IsNumber(temp)) {
                    ...
                    snprintf(temp_buffer, temp_buffer_size, "%.2f°C", temperature_celsius);
                } else {
            ...
            if (weather != NULL && cJSON_IsArray(weather)) {
                ...
                if (weather_item != NULL) {
                    ...
                    if (cJSON_IsString(weather_main)) {
                        // Format the weather type into the buffer
                        snprintf(main_buffer, main_buffer_size, "Weather: %s", weather_main->valuestring);
                    } else {
                        ESP_LOGE(TAG, "Weather Type not found in JSON");
                    }

                } else {
                    ESP_LOGE (TAG, "Weather Item Out of Bounds");
                }

            } else {
                ESP_LOGE(TAG, "Weather section not found in JSON");
            }
            cJSON_Delete(json);
        }
